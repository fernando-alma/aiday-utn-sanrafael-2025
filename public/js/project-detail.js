...
window.isProjectCreator = function (projectId, userEmail) {
  ...
  return creators.some(
    (c) => c.projectId == projectId && c.email === userEmail
  );
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (projectId) fetchProjectDetails(projectId);
    ...
    projectDetailContent.innerHTML =
      "<p>No se especificó ningún ID de proyecto.</p>";
  ...
  async function fetchProjectDetails(id) {
    try {
      ...
      if (data.success && data.project) {
        ...
        projectDetailContent.innerHTML = `<p>Error al cargar detalles: ${
  ...
  function renderProjectDetails(project) {
    ...
    const formatProjectDescription = (description) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (isFirstLine) {
          html += `<div class="project-title-numbered">${escapeHtml(
            line
          )}</div>`;
          isFirstLine = false;
          continue;
        ...
        if (bulletPoints.length > 0 && !hasProcessedInitialBullets) {
          html += `<ul class="project-bullet-points">${bulletPoints
            .map((p) => `<li>${p}</li>`)
            .join("")}</ul>`;
          bulletPoints = [];
        ...
        if (/^[•\-]\s/.test(line)) {
          if (bulletPoints.length === 0)
            html += `<ul class="project-bullet-points">`;
          bulletPoints.push(escapeHtml(line.replace(/^[•\-]\s/, "")));
          continue;
        }
        ...
        if (currentSection && line)
          html += `<div class="project-section-content">${escapeHtml(
            line
          )}</div>`;
        else if (line)
          html += `<div class="project-general-text">${escapeHtml(line)}</div>`;
      }
      ...
      return membersData
        ...
        .map((line) => {
          ...
          return name && linkedin
            ? { name: escapeHtml(name), linkedin: escapeHtml(linkedin) }
            : null;
        })
        .filter(Boolean);
    };
    ...
    projectDetailContent.innerHTML = `
      <div class="project-section">
          <div class="section-content">
            ${
              project.image
                ? `<img src="backend/public${escapeHtml(
                    project.image
                  )}" alt="Project Image" style="max-width:100%; height:auto; border-radius: 12px;">`
                : '<img src="public/images/fondos/fondo.jpg" alt="Project Image" style="max-width:100%; height:auto; border-radius: 12px;">'
            }
          </div>
        </div>

      <h2 class="form-title project-detail-title">${escapeHtml(
        project.title
      )}</h2>
      <div class="section-content"><time>${
        project.created_at ? dateFormat(project.created_at) : "N/A"
      }</time></div>
      ...
      <div class="project-details">
        ...
        <div class="project-section">
          ...
          <div class="section-content">${escapeHtml(
            detectStatus(project.status) || "N/A"
          )}</div>
        </div>
      ...
      button.addEventListener("click", async () => {
        if (!window.isAuthenticated || !window.isAuthenticated()) {
          if (typeof window.showLoginModal === "function")
            ...
            document.getElementById("joinStatus").textContent =
              "No se puede mostrar el modal de inicio de sesión.";
          return;
        }
        ...
        if (!userEmail || !userName) {
          joinStatus.textContent =
            "Por favor, inicia sesión para unirte a un proyecto.";
          return;
        }
        ...
        updateButtonStates(
          button,
          otherButton,
          action === "join" ? "Uniéndose..." : "Abandonando...",
          true
        );
        joinStatus.textContent = "";
    ...
    async function checkMembershipAndSetState() {
      ...
      try {
        ...
        if (data.success && Array.isArray(data.members)) {
          ...
          const functionalRole = window.getUserFunctionalRole(
            project.id,
            userEmail,
            backendRole
          );
  ...
  async function loadProjectMembers(projectId) {
    ...
    try {
      const response = await fetch(
        `${API_BASE}project/members?id=${projectId}`
      );
      if (!response.ok) {
      ...
      if (data.success && Array.isArray(data.members))
        ...
        membersList.innerHTML =
          '<p style="color:#a1a1a1; text-align:center;">No hay miembros en este proyecto.</p>';
    } catch (error) {
      console.error("Error cargando miembros:", error);
      membersList.innerHTML =
        '<p style="color:#a1a1a1; text-align:center;">No se pudieron cargar los miembros.</p>';
    }
  }
  ...
  function displayProjectMembers(members) {
    ...
    if (members.length === 0) {
      membersList.innerHTML =
        '<p style="text-align: center; color:#a1a1a1;">No hay miembros en este proyecto.</p>';
      return;
    }
    ...
    const membersHTML = members
      .map((member) => {
        ...
        const memberName = escapeHtml(
          member.name || member.user_name || "Usuario"
        );
        ...
        const memberRole =
          functionalRole === "owner" ? "Owner" : escapeHtml(backendRole);
  ...
  window.addEventListener("resize", () => {
    ...
    if (titleElement && project)
      titleElement.textContent = truncateTitleForMobile(project.title);
  });
});
...
function detectStatus(str) {
  if (str === "in_progress") return 'En Progreso';
}
